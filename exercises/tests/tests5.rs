...
unsafe fn modify_by_address(address: usize) {
    // SAFETY:
    // Caller guarantees the address points to valid, initialized u32
    // No other references exist per safety requirements
    // Memory is properly aligned (u32 has alignment 4)
    // Lifetime requirements are satisfied by caller
    unsafe {
